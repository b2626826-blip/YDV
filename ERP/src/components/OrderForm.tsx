...
export function OrderForm({ products, onCancel, onCreate, initialOrder, onUpdate }: OrderFormProps) {
  ...
  const allocationValid = totalQuantity > 0 && allocatedQuantity === totalQuantity && colorAllocationValid && draft.batches.length > 0 && draft.batches.every((batch) => getBatchQuantity(batch) > 0 && batch.dueDate && isDueDateValid(batch) && batch.destinationCountry.trim());
  ...
  const addBatch = () => updateDraft((current) => ({ ...current, batches: [...current.batches, { id: `draft-batch-${makeId()}`, batchNumber: current.batches.length + 1, productionLine: '', items: current.selectedColors.map((color) => ({ color, quantity: 0, completedQuantity: 0, defectiveQuantity: 0 })), dueDate: '', destinationCountry: '', note: '' }] }));

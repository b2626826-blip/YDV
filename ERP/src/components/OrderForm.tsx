...
export function OrderForm({ products, onCancel, onCreate }: OrderFormProps) {
  const [step, setStep] = useState(0);
  const [draft, setDraft] = useState<NewOrderDraft>(emptyDraft);
  const [selectedProductIds, setSelectedProductIds] = useState<string[]>([]);
  const [configuredOrders, setConfiguredOrders] = useState<ProductionOrder[]>([]);
  ...
  const beginConfiguration = (product: Product) => {
    setDraft({ ...emptyDraft, productId: product.id });
    setNotice('');
    setStep(2);
  };
  const toggleColor = (color: string) => updateDraft((current) => {
    ...
    return selected
      ...
      : { ...current, selectedColors: [...current.selectedColors, color], quantities: { ...current.quantities, [color]: current.quantities[color] ?? 0 }, batches: current.batches.map((batch) => ({ ...batch, items: [...batch.items, { color, quantity: 0, completedQuantity: 0 }] })) };
  });
  ...
  const addBatch = () => updateDraft((current) => ({ ...current, batches: [...current.batches, { id: `draft-batch-${makeId()}`, batchNumber: current.batches.length + 1, productionLine: '生產線 A', items: current.selectedColors.map((color) => ({ color, quantity: 0, completedQuantity: 0 })), dueDate: '', destinationCountry: '', note: '' }] }));
  ...
  return <main className="min-h-screen bg-slate-50"><header className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><div><button type="button" onClick={onCancel} className="text-sm font-semibold text-slate-500 hover:text-teal-700">← 返回 Dashboard</button><h1 className="mt-1 text-xl font-bold text-slate-900">新增生產訂單</h1></div><span className="rounded-full bg-teal-50 px-3 py-1 text-xs font-bold text-teal-700">已選 {cartProducts.length} 款鞋型</span></div></header><div className="mx-auto max-w-6xl px-5 py-8"><ol className="mb-8 grid grid-cols-6 gap-2">{steps.map((label, index) => <li key={label} className="min-w-0"><div className={`mb-2 h-1 rounded-full ${index <= step ? 'bg-teal-500' : 'bg-slate-200'}`} /><span className={`hidden text-xs font-semibold sm:block ${index === step ? 'text-teal-700' : 'text-slate-500'}`}>{index + 1}. {label}</span></li>)}</ol><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-7"><div className="mb-6"><p className="text-xs font-bold tracking-wider text-teal-700">STEP {step + 1} / {steps.length}</p><h2 className="mt-1 text-2xl font-bold text-slate-900">{steps[step]}</h2></div>
    ...
    {step === 1 && <div><p className="mb-4 text-sm text-slate-500">確認要建立的鞋型；所有鞋型會共用一張總訂單，並分別建立鞋型生產單。</p><div className="space-y-3">{cartProducts.map((product) => { const configuredOrder = configuredOrders.find((order) => order.product.id === product.id); return <article key={product.id} className="flex flex-col gap-4 rounded-xl border border-slate-200 p-4 sm:flex-row sm:items-center"><ProductMedia product={product} size="thumbnail" /><div className="min-w-0 flex-1"><p className="text-xs font-bold text-teal-700">{product.productCode}</p><p className="mt-1 font-bold text-slate-900">{product.productName}</p><p className="mt-1 text-xs text-slate-500">{configuredOrder ? `已設定 ${formatQuantity(configuredOrder.totalQuantity)} 雙` : '尚未設定顏色、數量與交貨批次'}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => beginConfiguration(product)} disabled={Boolean(configuredOrder)} className="rounded-lg border border-teal-600 px-3 py-2 text-sm font-bold text-teal-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400">{configuredOrder ? '已設定' : '設定此鞋型'}</button><button type="button" onClick={() => removeProduct(product.id)} className="rounded-lg px-3 py-2 text-sm font-bold text-rose-600 hover:bg-rose-50">移除</button></div></article>; })}</div></div>}

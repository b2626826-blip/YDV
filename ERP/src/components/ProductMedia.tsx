...
function ModelLoading() {
  ...
  return <Html center><span className="whitespace-nowrap rounded-md bg-slate-900/90 px-3 py-2 text-xs text-white">{t('載入 3D 模型…')} {Math.round(progress)}%</span></Html>;
}

class ModelErrorBoundary extends Component<{ children: ReactNode; message: string }, { failed: boolean }> {
  public override state = { failed: false };
  public static getDerivedStateFromError() { return { failed: true }; }
  public override render() {
    if (this.state.failed) return <Html center><span className="whitespace-nowrap rounded-md bg-rose-950/90 px-3 py-2 text-xs text-rose-100">{this.props.message}</span></Html>;
...
function ThreeDViewer({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return (
    <div className={`relative h-full overflow-hidden rounded-xl bg-slate-950 ${compact ? 'min-h-40' : 'min-h-[280px]'}`} aria-label={t('{code} 3D 鞋底檢視器', { code: product.productCode })}>
      <Canvas camera={{ fov: 35, position: initialCameraPosition }} dpr={[1, 2]} onCreated={() => setHasLoaded(true)}>
        ...
        <ModelErrorBoundary message={t('模型無法載入')}>
      ...
      {!compact && <span className="pointer-events-none absolute bottom-3 left-3 text-xs text-slate-300">{t('拖曳旋轉 · 滾輪縮放')}</span>}
      <button type="button" className={`absolute right-3 rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white backdrop-blur hover:bg-white/20 ${compact ? 'bottom-2' : 'bottom-3'}`} onClick={resetView} disabled={!hasLoaded}>↺ {t('Reset View')}</button>
    </div>
  );
}
...
export function ProductMedia({ product, size = 'card', interactive = false, className = '' }: ProductMediaProps) {
  ...
  return <div className={`${dimensions} overflow-hidden rounded-xl ${className}`}>{hasImageError ? <ImageFallback code={product.productCode} /> : <img src={product.mediaUrl} alt={t('{name} 鞋底', { name: product.productName })} className="h-full w-full object-cover" onError={() => setHasImageError(true)} />}</div>;
}

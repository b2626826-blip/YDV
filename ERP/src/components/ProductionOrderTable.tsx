...
export function ProductionOrderTable({ orders, onOpen }: { orders: ProductionOrder[]; onOpen: (id: string) => void }) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="hidden min-w-[1240px] grid-cols-[1.2fr_2.2fr_1.1fr_1.3fr_1.5fr_1.4fr_1.4fr] border-b border-slate-100 bg-slate-50 px-5 py-3 text-xs font-semibold text-slate-500 xl:grid"><span>訂單總編號</span><span>產品</span><span>顏色</span><span>生產進度</span><span>目前狀態</span><span>下一批交貨</span><span>目的地</span></div><div className="flex flex-col gap-3 bg-slate-50 p-3 xl:block xl:bg-white xl:p-0">{orders.map((order) => {
